...
st.set_page_config(layout="wide", page_title="CLIP DBSCAN Explorer")
...
def compute_dbscan(embeddings, eps, min_samples, metric="cosine"):
    db = DBSCAN(eps=float(eps), min_samples=int(min_samples), metric=metric, n_jobs=-1)
...
eps = st.sidebar.slider("DBSCAN eps (cosine distance)", min_value=0.000, max_value=1.000, value=0.20, step=0.005, format="%.3f")
min_samples = st.sidebar.slider("DBSCAN min_samples", min_value=1, max_value=50, value=5, step=1)
...
# Main UI
# -------------------------
st.title("CLIP + DBSCAN interactive explorer")
...
with col_right:
    ...
    # run DBSCAN on current eps/min_samples
    labels = compute_dbscan(embeddings_for_clustering, eps=eps, min_samples=min_samples, metric="cosine")
...
st.sidebar.markdown("---")
st.sidebar.write("Made with ❤️ — drag the sliders to change eps/min_samples and explore clusters live.")
